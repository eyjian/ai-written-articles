...
- **`DeerFlow` 更像运行时级 harness。** 它不是陪开发者在编辑器里写代码，而是给 Agent 提供独立运行、调度、记忆和隔离执行的底座。

如果只记一句，那就是：**`Trae` 和 `CodeBuddy` 是在 IDE 里把 harness 练起来，`DeerFlow` 是把 harness 做成系统。**

## 第一章：先别急着比产品，先把"在比什么"说清

`Harness Engineering` 这个词，最近被讨论得很多。Birgitta Böckeler 于 2026 年 4 月在 Martin Fowler 网站发表的 [Harness engineering for coding agent users](https://martinfowler.com/articles/harness-engineering.html) 里，把这套思路讲得很清楚。
...
所以如果问题是：**"`使用CodeBuddy实践Harness-Engineering.md` 里那套思路，能不能搬到 `Trae` 上？"**答案是：**可以，而且做法基本能平移。** 只需要把：
...
- **至少一个可用的大模型 API Key。** `DeerFlow` 不是离线玩具，先得跑 `make config` 生成 `.env` 和 `config.yaml`，再把模型密钥真正配进去；如果还想让它联网搜索，还得额外准备 `Tavily` 一类搜索服务的 Key。
...
这几条放在一起，就更容易理解它和 `CodeBuddy`、`Trae` 的差别了：**`DeerFlow` 不需要先依附某个 IDE，也不要求必须有 `CodeBuddy` 这样的工具；它更像一套自己就能独立启动的服务。CLI 主要是拿来安装、检查和启动，真正的使用入口是它自己的 Web UI。**

如果看过 `WorkBuddy` 这类产品，对 `DeerFlow` 的第一眼印象大概会很像：左边是会话和智能体入口，中间是任务工作区，`skill` 是显性概念。光看界面形态，它就更像一个独立的 Agent 工作台，而不是 IDE 里的侧边助手。
...
- **`CodeBuddy` / `Trae`** 像骑在马上，手里多了一套更靠谱的缰绳、路线卡和检查表。
- **`DeerFlow`** 像不只是拿着缰绳，而是连马厩、调度系统、补给站和多匹马协同机制都一起搭好了。

所以两者不是谁替代谁。而是一个偏**"人在环上的 IDE 纪律化协作"**，另一个偏**"Agent 可独立运行的基础设施工程"**。

## 第五章：把 `Trae` 也放进来，三者到底该怎么选

如果把 `Trae` 也一起拉进来，最实用的看法不是"谁更强"，而是"现在处在哪个阶段"。
...
| 误区 | 为什么不对 | 更准确的理解 |
|------|------------|--------------|
| **"`Trae` 也有规则和计划，所以它和 `DeerFlow` 是一类。"** | 规则和计划只覆盖 harness 的前半段，不代表已经有了隔离、记忆和运行时底座 | `Trae` 更像 IDE 级实践，`DeerFlow` 是运行时级系统 |
...
如果今天就想开始，先在 `CodeBuddy` 或 `Trae` 里把规则、计划、验收、复检这些基本功练顺。真遇到长周期、多 Agent、独立执行的瓶颈，再考虑 `DeerFlow`。

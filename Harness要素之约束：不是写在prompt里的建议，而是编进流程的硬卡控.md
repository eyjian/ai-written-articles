...
这两者的差距不是程度差异，是机制差异。口头嘱咐再多，也抵不过一道物理门禁。
...
这些不是模型笨。恰恰相反，模型"太聪明"了——它会自作主张地优化、会合理化自己的越界行为、会在上下文压力下走捷径。
...
上面四种约束的代码都是跑在独立编排层里的。但如果你用的是 CodeBuddy CLI（v1.16.0+）或 Claude Code，有些约束可以直接通过 **Hooks** 落地，不需要自己写编排层程序。
...
判断标准很简单：**如果 Agent 做错了一步，你能接受的修复成本有多高？** 成本低，prompt 级 rule 够用。成本高，就必须上硬约束。
...
- **约束 + 可观测性**：约束执行了多少次、拦截了哪些动作、哪些 Agent 最容易触发拦截——这些数据让你知道约束设得对不对、够不够。

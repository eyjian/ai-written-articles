...
| **用户建模** | 基于 Honcho 的辩证用户建模（通过多轮对立假设逐步构建用户画像），兼容 agentskills.io（一个技能共享平台） |

**为什么重要：** 这套机制解决的是 Agent 的"一次性问题"。没有学习循环的 Agent，本质上是个高级工具——用完即弃，不会成长。有了学习循环，Agent 会随着使用变成一个越来越适配使用场景、越来越能干的长期协作者。
...
**是什么：** 支持批量轨迹生成、基于 Atropos 的 RL 训练管道、轨迹压缩。
...
> **从 OpenClaw 迁移？** OpenClaw 是 Hermes Agent 的前身项目。Hermes Agent 会自动检测 `~/.openclaw` 目录并提供迁移引导，包括 `SOUL.md`（个性配置）、记忆、技能、API 密钥等，无需手动操作。
...
| **学习循环** | 内置，技能自动创建和改进 | 无内置学习循环 | 无内置学习循环 |
| **跨会话记忆** | 内置记忆系统 + 会话检索 | 通过 `CLAUDE.md` 和 memory 工具实现 | 内置记忆功能（Plus 用户） |
...
这不是"谁好谁差"的问题，而是定位不同。Claude Code 在代码领域深耕，ChatGPT 在通用对话上体验成熟，Hermes Agent 的差异化在于**自我进化 + 自托管 + 多平台**。选哪个取决于核心需求。

...
> 同样是 5 个 Agent，换一种拓扑就从"流水线"变成了"团队"。手把手教你在 CodeBuddy 里构建一个 Agent 之间能直接对话、自主决策的真正 Agent Team。

---

## 引言：上一篇我们做了什么，这一篇要做什么不同

上一篇《使用 CodeBuddy IDE 构建 Sub-Agent》里，我们用 CodeBuddy Skills 搭了 5 个独立角色（选题、大纲、写稿、审稿、润色），你手动触发每一个，控制全流程。那是 **Sub-Agent 流水线**——你是老板，角色是听话的下属。

这一篇，我们用**同样的 5 个角色**，做一件完全不同的事——把它们变成一个**真正的 Agent Team**。区别在哪？

| 维度 | 上一篇（Sub-Agent） | 本篇（Agent Team） |
|------|--------------------|--------------------|
| **谁控制流程** | 你手动触发每个 Skill | 团队自行协作，你只做确认 |
| **角色间通信** | 互不交流，各自对你汇报 | 可以互相直接对话 |
| **决策权** | 全在你手里 | 分散到每个角色 |
...
> **工具给了你网状通信的能力，但你自己画了一个星型拓扑。** 要做 Agent Team，关键不是换工具，是换 prompt 设计。
...
|------|------------------------|------------------------|
| **选题侦察员** | 只输出方案给你，等你确认 | 输出方案给你确认后，**自行通知大纲架构师** |
| **大纲架构师** | 只输出大纲给你，等你指令 | 如果选题有问题，**直接找选题侦察员讨论调整** |
| **初稿写手** | 按章节写完等你确认 | 发现大纲不对时**直接找大纲架构师讨论** |
| **技术审稿人** | 输出审稿报告等你决定退不退回 | **自己决定退不退回，直接找写手修改** |
| **终稿润色师** | 润色完等你终审 | 发现技术错误时**直接反馈给审稿人** |
...
#### 变化 2：决策权从"集中在你"变成"分散到角色"

Sub-Agent 版本：
```
reviewer 输出审稿报告 → 你看报告 → 你决定退不退回 → 你触发 writer 修改
```
...
**真正需要你拍板的核心确认节点是三个**：选题确认、大纲确认、终稿确认。其余的——退回修改、讨论调整、反馈问题——团队内部自行解决；另外，`writer` 还会按章节同步进度，但不会阻塞主流程。
...
> 这里展示的是本文项目里的组织方式，方便你照着复现；它不是 CodeBuddy IDE 官方规定的唯一目录结构。
...
| 节点 | 你做什么 | 之后谁接手 |
|------|---------|-----------|
| 选题确认 | scout 给出方案，你选一个 | scout 自行通知 architect |
| 大纲确认 | architect 给出大纲，你审批 | architect 自行通知 writer |
| 终稿确认 | polisher 润色完成，你终审 | 发布 |

另外，当前实现里 `writer` 还会按章节同步进度，但主流程不会因此停下来等你逐章批准。
...
| **通信规则** | 每个 Skill 只跟你对话 | Agent 之间可直接 `send_message` |
| **决策权** | 全在你手里 | 分散到各角色（reviewer 自己退回） |
| **流程** | 你控制顺序，严格串行 | 动态——可并行、可回退 |
| **prompt 核心差异** | 输出结果等用户决定 | 自主判断 + 直接联系相关角色 |
| **协调者角色** | 你就是老板 | 在本文实现里主要负责启动和传话 |
